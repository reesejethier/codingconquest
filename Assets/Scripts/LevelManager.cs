...
public class LevelManager : MonoBehaviour {
	...
	private bool isRespawning;//spawn Mario?
	...
	public void MarioRespawn(bool timeup = false) {
		if (!isRespawning) {
			isRespawning = true;
			Debug.Log("LevelManager: Mario is Responding");
	...
	public Vector3 FindSpawnPosition() {
		...
		//	spawnPosition = new Vector3(PlayerPrefs.GetFloat("posx"), PlayerPrefs.GetFloat("posy"), PlayerPrefs.GetFloat("posz"));
				spawnPosition = GameObject.Find ("Spawn Points").transform.GetChild (t_GameStateManager.spawnPointIdx).transform.position;
			
			Debug.Log("LevelManager: Finding Spawn Position" + spawnPosition);
			Debug.Log("LevelManager: SpawnPoint" +t_GameStateManager.spawnFromPoint);
	...
	public void MarioCompleteLevel() {
		...
		// flag to unlock next level here???
	}
