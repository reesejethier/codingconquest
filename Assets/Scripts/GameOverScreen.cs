...
public class GameOverScreen : MonoBehaviour {
	...
	void Start() {
		...
		Debug.Log(" I'm dead lol, Menu screen is loading");
		//LoadMainMenu(gameOverMusicSource.clip.length);
		LoadMap(gameOverMusicSource.clip.length);
	...
	void Update() {
		if (Input.GetButton("Pause")) {
			LoadMainMenu();
			Debug.Log("Returning after game over...");
		}
	}
	...
	//load back to the map
	void LoadMap(float delay = 0)
	{
		StartCoroutine(LoadSceneDelayCo("WorldMap", delay));
	}
